...
enum {
  ...
  ChordTypeMinor=1,
  ChordTypeSeventh=2,
  ChordTypeMinorSeventh=3,
  ChordTypeAugmented,
  ChordTypeDiminished,
  ChordTypeAugmentedMajorSeventh,
  ChordTypeSuspendedFourth,
  ChordTypeSuspendedSecond,
  ChordTypeDiminishedSeventh,
  ChordTypeDiminishedMajorSeventh,
  ChordTypeDominantSeventhFlatFive,
  ChordTypeHalfDiminishedSeventh,
  ChordTypeMinorMajorSeventh
  };

typedef struct {char* d;int v[4];} ChordDistances;
ChordDistances ChordDistanceList[]={
  {"",0,4,7,-1},
  {"m",0,3,7,-1},
  {"7",0,4,7,10},
  {"m7",0,3,7,10},
...
void reportChordDistances(int id) {
  int i;
  printf("%s:",ChordDistanceList[id].d);
  for (i=0;i<4;i++) if (ChordDistanceList[id].v[i]!=-1) printf("%d ",ChordDistanceList[id].v[i]);
  printf("\n");
}
...
void storePercSeq(int id) {
  ...
  printf("memcpy start %p %p %d\n",percSeq[id].drums,drums,(int)(drumsSize*sizeof(NoteEvent)));
  memcpy(percSeq[id].drums,drums,(int)(drumsSize*sizeof(NoteEvent)));
...
void restorePercSeq(int id) {
  ...
  printf("mem restore start %p %p %d\n",drums,percSeq[id].drums,(int)(drumsSize*sizeof(NoteEvent)));
...
int freeChordNoteSeq() {
  int i;
  for (i=0;i<128;i++) if (chordNoteSeq[i].chords==NULL) return i;
  return -1;
}
...
int storeFreeChordNoteSeq() {
  int freeid=freeChordNoteSeq();
  if (freeid>=0) storeChordNoteSeq(freeid); 
  return freeid;
}
...
void addChord(int note,int time) {
  if (note%4==3) {
    printf("we are on the fourth note %d type is %d %s chord is:",note,chordType,ChordDistanceList[chordType].d);
    int i;
    for (i=0;i<4;i++) printf("%d ",ChordDistanceList[chordType].v[i]);
    printf("\n");
    
    if (ChordDistanceList[chordType].v[3]==-1) return;
      //without adding this chord does not have a fourth note
  }
...
int idToNote(int id) {
  int r=0;
  if (ChordDistanceList[chordType].v[id%4]!=-1) r=12*(id/4)+ChordDistanceList[chordType].v[id%4];
  else r=12+12*(id/4)+ChordDistanceList[chordType].v[0];
...
int noteToId(int note) {
  int octave=note/12;
  int base=(activeChord&0xf);
  if (note%12-base==0) return octave*4;
  if (note%12-base==ChordDistanceList[chordType].v[1]) return octave*4+1;
  if (note%12-base==ChordDistanceList[chordType].v[2]) return octave*4+2;
  if (note%12-base==ChordDistanceList[chordType].v[3]) return octave*4+3;
  //we didnt finde anything? return -1;
  return -1;
}
...
void chordIDcolor(int id) {
     if (id==0) uicolor(0xffffff); else if (id==1) uicolor(0x00ff00);
    else if (id==2) uicolor(0x003fff); else if (id==3) uicolor(0xff0000); 
}
...
void showChordSequence() {
  ...
  for (i=0;i<chordsDuration;i++) {
    ...
    uiline(1+chords[i].time/10000,590-3*chords[i].note,1+chords[i].time/10000,595-3*chords[i].note);
    uiline(1+chords[i].time/10000,600-3*chords[i].note,1+chords[i].time/10000,605-3*chords[i].note);
...
void chordHelp() {
    ...
    sprintf(report,"pg up/down: move cursor");
    uitext(500,120,report);
    sprintf(report,"insert    : toggle cursor/time insert");
    uitext(500,130,report);
    sprintf(report,"home      : toggle metronome");
    uitext(500,140,report);
    sprintf(report,"Current chord %c%c %s (F1 to change)",notedescr[activeChord*2],notedescr[activeChord*2+1],ChordDistanceList[chordType].d);
    uitext(500,180,report);
    sprintf(report,"end       : sync cursor");
    uitext(500,160,report);
    uicolor(0);
    uirect(550,200,700,300);
    chordIDcolor(0);
    uitext(600,220,"base");
    chordIDcolor(1);
    uitext(600,230,"middle");
    chordIDcolor(2);
    uitext(600,240,"fifth");
    chordIDcolor(3);
    uitext(600,250,"extend");
}
...
void chordsSeq(){
  ...
  while (!q) {
   ...
   { 
    ...
    if (event.xkey.keycode==67) {
      int value=chordMenu();
      activeChord=(value & 0x0f)%12;
      chordType=(value & 0xf0)>>4;
      clearChordScreen();
      chordHelp();
    }
    if (x>0 && y<=4) {
...
int chordNoteExists(ChordNoteSequence *p,int note,int time) {
  int i;
  for (i=0;i<p->chordsDuration;i++) if (p->chords[i].time==time && p->chords[i].note==note) return 1;
  return 0;
}


int sameChordNoteSequences(ChordNoteSequence *p1,ChordNoteSequence *p2) {
  if (p1==NULL && p2==NULL) return 1;
  if (p1==NULL || p2==NULL) return 0;
  int i;
  for (i=0;i<p1->chordsDuration;i++) if (!chordNoteExists(p2,p1->chords[i].note,p1->chords[i].time)) return 0;
  for (i=0;i<p2->chordsDuration;i++) if (!chordNoteExists(p1,p2->chords[i].note,p2->chords[i].time)) return 0;
  return 1;
}

int inChordNoteSequence(ChordNoteSequence *p) {
  int i;
  for (i=0;i<128;i++) if (chordNoteSeq[i].chords!=NULL) if (sameChordNoteSequences(&(chordNoteSeq[i]),p)) return 1+i;
  return 0;
}
...
void addSongNote(int note) {
  ...
  song[songDuration].instrument=instrument;
  int bar,q,tick;
...
int chordMenu() {
  ...
  SetKey(&menukeys[7],500,50,100,20,4,114,"-> next type");
  SetKey(&menukeys[8],500,80,100,20,4,113,"<- prev type");
  SetKey(&menukeys[9],500,110,100,20,4,111,"^:#");
  SetKey(&menukeys[10],500,140,100,20,4,36,"enter: done");
  SetKey(&menukeys[11],500,20,100,20,4,65,"space: major");
  //for (i=0;i<12;i++) menukeys[i].pressed=0;
  char *ctr="1234567890-=<";
  char keydescr[16];
  for (i=12;i<12+ChordTypeMinorMajorSeventh;i++) {
    sprintf(keydescr,"%c:%s",ctr[i-12],ChordDistanceList[i-12].d);
    SetKey(&menukeys[i],450,200+30*(i-12),100,20,4,10+(i-12),keydescr);
  }
  q=0;base=0;
  type=chordType;sharp=0;
  while (!q) {
    char report[16];
    uievent();
    XEvent event=uilastevent();
    uiclear(0xffffff);    
    for (i=0;i<13+ChordTypeMinorMajorSeventh;i++) {CheckKey(&menukeys[i],event); ShowKey(&menukeys[i]);uiflush();}
    int values[]={0,2,4,5,7,9,11};
    for (i=0;i<7;i++) if (menukeys[i].pressed) base=values[i];
    if (menukeys[7].pressed) type++;
    if (menukeys[8].pressed) type+=ChordTypeMinorMajorSeventh;
    type%=ChordTypeMinorMajorSeventh+1;
    if (menukeys[9].pressed) sharp=!sharp;
    if (menukeys[10].pressed) {q=1;}
    if (menukeys[11].pressed) {type=ChordTypeMajor;}
    for (i=0;i<ChordTypeMinorMajorSeventh;i++) if (menukeys[12+i].pressed) type=i;
    r=base+sharp;
    int notes=4;
    if (ChordDistanceList[type].v[3]==-1) notes=3;
    if (notes==4) 
    sprintf(report,"%c%c%s {%d,%d,%d,%d}",notedescr[r*2],notedescr[r*2+1],ChordDistanceList[type].d,
      base+ChordDistanceList[type].v[0],
      base+ChordDistanceList[type].v[1],
      base+ChordDistanceList[type].v[2],
      base+ChordDistanceList[type].v[3]
    );
    else sprintf(report,"%c%c%s {%d,%d,%d}",notedescr[r*2],notedescr[r*2+1],ChordDistanceList[type].d,
      base+ChordDistanceList[type].v[0],
      base+ChordDistanceList[type].v[1],
      base+ChordDistanceList[type].v[2]
    );
...
void saveSong(char *filename) {
  ...
  char infotext[64];
  if (filename!=NULL) sprintf(reply,"%s",filename);
  else textAnswer("save song to filename:",150,200);
  FILE *file;
  file=fopen(reply,"wb");
  if (!file) {
...
void mainmenu() {
  ...
  SetKey(&menukeys[8],300,290,200,20,4,18,"9: import accomp. from MIDI");
  uiclear(0xffffff);
  q=0;
  while (!q) {
    uievent();
    ...
    if (menukeys[4].pressed) {saveSong(NULL);q=1;}
    ...
    if (menukeys[8].pressed) {importSequences();q=1;}
  }
}
...
void showSong() {
  ...
  for (i=0;i<accompanDuration;i++) {
	...
	printf("value is %d v is %d minor is %c seventh is %c chord type is %s\n",value,v,minor,seventh,ChordDistanceList[ct].d);
	sprintf(report,"%c%c%s",notedescr[v*2],notedescr[v*2+1],ChordDistanceList[ct].d);
      } else if (type==TextEvent) {
	location=14;
...
void echoSong() {
  ...
  for (i=0;i<songDuration;i++) {
    ...
    int ctime=song[i].time;
    int cduration=song[i].duration;
    int cinstrument=song[i].instrument;
    if (cinstrument!=instrument) {
...
void layoutSong() {
  ...
  for (i=0;i<songDuration;i++) {
    ...
    int cduration=song[i].duration;
    int cinstrument=song[i].instrument;
    addMidiEvent(ctime,0,SND_SEQ_EVENT_PGMCHANGE,cinstrument);
...
void procNoteTrackLength(int time,int type,int channel,int note,int vel,void *data) {
...
void loadMidi(char *filename) {
  ...
  for (i=0;i<nTracks;i++) {
    ...
    parseTrack(&(tracks[i]),procNoteTrackLength,NULL,NULL);
    printf("track %d done\n",i);
  }
...
void importDrums() {
  ...
  MidiLibGoSilent(1);
  int firstid=storeFreePercSeq();
  if (firstid!=-1) {
    currenttime.bar=0;currenttime.q=0;currenttime.tick=0;
    addAccompan(SelectPercussion,firstid);
    addAccompan(PercussionFlag,1);
  }
  else ("printf failed to store first pattern\n");
  ...
  for (i=0;i<songMeters;i++) {
    ...
    if (bp) printf("%d already in bank %d\n",i,bp); else {
      ...
      int newid=storeFreePercSeq();
      if (newid==-1) printf("out of free banks storing %d\n",i);
      else {
	currenttime.bar=i;
	addAccompan(SelectPercussion,newid);
      }
      //copyPercussionSequence(&lastSequence,&importedSequence);
    }
  }
...
int main(int argc,char **argv) {
  ...
  ChordTypeMinor ,%d\n\
  ChordTypeSeventh ,%d\n\
  ChordTypeMinorSeventh ,%d\n\
  ChordTypeAugmented ,%d\n\
  ChordTypeDiminished ,%d\n\
  ChordTypeAugmentedMajorSeventh ,%d\n\
  ChordTypeSuspendedFourth ,%d\n\
  ChordTypeSuspendedSecond ,%d\n\
  ChordTypeDiminishedSeventh ,%d\n\
  ChordTypeDiminishedMajorSeventh,%d\n\
  ChordTypeDominantSeventhFlatFive,%d\n\
  ChordTypeHalfDiminishedSeventh,%d\n\
  ChordTypeMinorMajorSeventh %d\n",
  ChordTypeMajor ,
  ChordTypeMinor ,
  ChordTypeSeventh ,
 ...
 reportChordDistances(ChordTypeMajor);
  reportChordDistances(ChordTypeMinor);
  reportChordDistances(ChordTypeSeventh);
  reportChordDistances(ChordTypeMinorSeventh);
  reportChordDistances(ChordTypeAugmented);
  reportChordDistances(ChordTypeDiminished);
  reportChordDistances(ChordTypeAugmentedMajorSeventh);
  reportChordDistances(ChordTypeSuspendedFourth);
  reportChordDistances(ChordTypeSuspendedSecond);
  reportChordDistances(ChordTypeDiminishedSeventh);
  reportChordDistances(ChordTypeDiminishedMajorSeventh);
  reportChordDistances(ChordTypeDominantSeventhFlatFive);
  reportChordDistances(ChordTypeHalfDiminishedSeventh);
  reportChordDistances(ChordTypeMinorMajorSeventh);
